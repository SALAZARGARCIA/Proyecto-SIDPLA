...
if (isset($_POST["registrarVenta"])) {
 ...
 values($cart_id,$c[product_id],$c[q],$vs)");
        ...
        print "<script>alert('Venta procesada exitosamente');window.location='../Vista/administrador/productos.php';</script>";
    } else {
...
if (isset($_POST["comentar"])) { // Verifica si el botón oprimido es el de comentar
 ...
 values('$com','$idper','$tdocper',NOW())");
    ...
    print "<script>alert(\"Gracias por tu opinión\");window.location='../Vista/administrador/coment.php';</script>";
}
...
if (isset($_POST["cambio_est_dom"])) {
    ...
    print "<script>alert(\"Domicilio entregado exitosamente\");window.location='../Vista/administrador/empleado.php';</script>";
